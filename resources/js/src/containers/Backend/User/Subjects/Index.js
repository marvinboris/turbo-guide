...
class Index extends Component {
    ...
    render() {
        ...
        const feature = features.find(f => f.prefix === 'subjects');
        ...
        const data = subjects.map(subject => {
            return updateObject(subject, {
                action: <div className="text-center">
                    <Link to={`/user/subjects/${subject.id}`} className="mr-2">
                        <FontAwesomeIcon icon={faEye} className="text-green" fixedWidth />
                    </Link>
                    {JSON.parse(feature.permissions).includes('u') && <Link to={`/user/subjects/${subject.id}/edit`} className="mx-1">
                        <FontAwesomeIcon icon={faEdit} className="text-blue" fixedWidth />
                    </Link>}
                    {JSON.parse(feature.permissions).includes('d') && <span className="mx-1"><Delete deleteAction={() => this.props.delete(subject.id)}><FontAwesomeIcon icon={faTrash} className="text-red" fixedWidth /></Delete></span>}
                </div>,
            });
        });
        ...
        const content = (
            <>
                <Row>
                    <List array={data} loading={loading} data={JSON.stringify(subjects)} get={this.props.get} total={total} bordered add={add} link="/user/subjects/add" icon={faBook} title={index} className="shadow-sm"
                        fields={[
