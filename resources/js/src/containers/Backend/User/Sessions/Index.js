...
class Index extends Component {
    ...
    render() {
        ...
        const feature = features.find(f => f.prefix === 'sessions');
        ...
        const data = sessions.map(session => {
            return updateObject(session, {
                action: <div className="text-center">
                    <Link to={`/user/sessions/${session.id}`} className="mr-2">
                        <FontAwesomeIcon icon={faEye} className="text-green" fixedWidth />
                    </Link>
                    {JSON.parse(feature.permissions).includes('u') && <Link to={`/user/sessions/${session.id}/edit`} className="mx-1">
                        <FontAwesomeIcon icon={faEdit} className="text-blue" fixedWidth />
                    </Link>}
                    {JSON.parse(feature.permissions).includes('d') && <span className="mx-1"><Delete deleteAction={() => this.props.delete(session.id)}><FontAwesomeIcon icon={faTrash} className="text-red" fixedWidth /></Delete></span>}
                </div>,
            });
        });
        ...
        const content = (
            <>
                <Row>
                    <List array={data} loading={loading} data={JSON.stringify(sessions)} get={this.props.get} total={total} bordered add={add} link="/user/sessions/add" icon={faSchool} title={index} className="shadow-sm"
                        fields={[
