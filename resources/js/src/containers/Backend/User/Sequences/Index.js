...
class Index extends Component {
    ...
    render() {
        ...
        const feature = features.find(f => f.prefix === 'sequences');
        ...
        const data = sequences.map(sequence => {
            return updateObject(sequence, {
                action: <div className="text-center">
                    ...
                    {JSON.parse(feature.permissions).includes('u') && <Link to={`/user/sequences/${sequence.id}/edit`} className="mx-1">
                        <FontAwesomeIcon icon={faEdit} className="text-blue" fixedWidth />
                    </Link>}
                    {JSON.parse(feature.permissions).includes('d') && <span className="mx-1"><Delete deleteAction={() => this.props.delete(sequence.id)}><FontAwesomeIcon icon={faTrash} className="text-red" fixedWidth /></Delete></span>}
                </div>,
            });
        });
