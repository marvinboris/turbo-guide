...
class Index extends Component {
    ...
    render() {
        ...
        const data = levels.map(level => {
            return updateObject(level, {
                ...
                action: <div className="text-center">
                    <Link to={`/admin/levels/${level.id}`} className="mr-2">
                        <FontAwesomeIcon icon={faEye} className="text-green" fixedWidth />
                    </Link>
                    <Link to={`/admin/levels/${level.id}/edit`} className="mr-2">
                        <FontAwesomeIcon icon={faEdit} className="text-brokenblue" fixedWidth />
                    </Link>
                    <span className="mx-1"><Delete deleteAction={() => this.props.delete(level.id)}><FontAwesomeIcon icon={faTrash} className="text-red" fixedWidth /></Delete></span>
                </div>,
            });
        });
        ...
        const content = (
            <>
                <Row>
                    <List array={data} loading={loading} data={JSON.stringify(levels)} get={this.props.get} total={total} bordered add={add} link="/admin/levels/add" icon={faLayerGroup} title={index} className="shadow-sm"
                        fields={[
