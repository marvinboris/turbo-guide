...
class Notification extends Component {
    ...
    render() {
        let {
            ...
            backend: { notifications: { loading, error, message, notification } },
            auth: { data: { name } }
        } = this.props;
        let content = null;
        ...
        return (
            <>
                <TitleWrapper>
                    <Breadcrumb items={[{ to: '/restaurant/notifications', content: index }]} main={show} icon={faBell} />
...
const mapDispatchToProps = dispatch => ({
    show: id => dispatch(actions.getNotification(id)),
});
