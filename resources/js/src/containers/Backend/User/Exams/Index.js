...
class Index extends Component {
    ...
    render() {
        ...
        const feature = features.find(f => f.prefix === 'exams');
        ...
        const data = exams.map(exam => {
            return updateObject(exam, {
                action: <div className="text-center">
                    <Link to={`/user/exams/${exam.id}`} className="mr-2">
                        <FontAwesomeIcon icon={faEye} className="text-green" fixedWidth />
                    </Link>
                    {JSON.parse(feature.permissions).includes('u') && <Link to={`/user/exams/${exam.id}/edit`} className="mx-1">
                        <FontAwesomeIcon icon={faEdit} className="text-blue" fixedWidth />
                    </Link>}
                    {JSON.parse(feature.permissions).includes('d') && <span className="mx-1"><Delete deleteAction={() => this.props.delete(exam.id)}><FontAwesomeIcon icon={faTrash} className="text-red" fixedWidth /></Delete></span>}
                </div>,
            });
        });
        ...
        const content = (
            <>
                <Row>
                    <List array={data} loading={loading} data={JSON.stringify(exams)} get={this.props.get} total={total} bordered add={add} link="/user/exams/add" icon={faSchool} title={index} className="shadow-sm"
                        fields={[
