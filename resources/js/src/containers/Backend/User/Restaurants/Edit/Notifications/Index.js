...
class Notifications extends Component {
    ...
    render() {
        let {
            ...
            backend: { restaurants: { loading, error, message, notifications } },
        } = this.props;
        let content = null;
            ...
            if (notifications) {
                const notificationsData = notifications.map(notification => {
                    ...
                    return <ListGroupItem key={"list-group-item-notification-" + notification.id} color="soft" className="border-darkblue-20">
                        <Link to={"/user/restaurants" + this.props.match.params.restaurant + "/notifications/" + notification.id} className="text-reset text-decoration-none">
...
const mapDispatchToProps = dispatch => ({
    get: () => dispatch(actions.getRestaurantsEdit('notifications')),
});
