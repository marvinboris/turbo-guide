...
class Index extends Component {
    ...
    render() {
        ...
        const feature = features.find(f => f.prefix === 'marks');
        ...
        const data = marks.map(mark => {
            return updateObject(mark, {
                action: <div className="text-center">
                    ...
                    {JSON.parse(feature.permissions).includes('u') && <Link to={`/user/marks/${mark.id}/edit`} className="mx-1">
                        <FontAwesomeIcon icon={faEdit} className="text-blue" fixedWidth />
                    </Link>}
                    {JSON.parse(feature.permissions).includes('d') && <span className="mx-1"><Delete deleteAction={() => this.props.delete(mark.id)}><FontAwesomeIcon icon={faTrash} className="text-red" fixedWidth /></Delete></span>}
                </div>,
            });
        });
        ...
        const content = (
            <>
                <Row>
                    <List array={data} loading={loading} data={JSON.stringify(marks)} get={this.props.get} total={total} bordered add={add} link="/user/marks/add" icon={faMarker} title={index} className="shadow-sm"
                        fields={[
