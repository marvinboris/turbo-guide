...
class Index extends Component {
    ...
    render() {
        ...
        const data = years.map(year => {
            return updateObject(year, {
                action: <div className="text-center">
                    <Link to={`/user/years/${year.id}`} className="mr-2">
                        <FontAwesomeIcon icon={faEye} className="text-green" fixedWidth />
                    </Link>
                    {JSON.parse(feature.permissions).includes('u') && <Link to={`/user/years/${year.id}/edit`} className="mx-1">
                        <FontAwesomeIcon icon={faEdit} className="text-blue" fixedWidth />
                    </Link>}
                    {JSON.parse(feature.permissions).includes('d') && <span className="mx-1"><Delete deleteAction={() => this.props.delete(year.id)}><FontAwesomeIcon icon={faTrash} className="text-red" fixedWidth /></Delete></span>}
                </div>,
            });
        });
        ...
        const content = (
            <>
                <Row>
                    <List array={data} loading={loading} data={JSON.stringify(years)} get={this.props.get} total={total} bordered add={add} link="/user/years/add" icon={faCalendar} title={index} className="shadow-sm"
                        fields={[
