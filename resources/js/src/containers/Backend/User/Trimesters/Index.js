...
class Index extends Component {
    ...
    render() {
        ...
        const feature = features.find(f => f.prefix === 'trimesters');
        ...
        const data = trimesters.map(trimester => {
            return updateObject(trimester, {
                ...
                action: <div className="text-center">
                    <Link to={`/user/trimesters/${trimester.id}`} className="mr-2">
                        <FontAwesomeIcon icon={faEye} className="text-green" fixedWidth />
                    </Link>
                    {JSON.parse(feature.permissions).includes('u') && <Link to={`/user/trimesters/${trimester.id}/edit`} className="mr-2">
                        <FontAwesomeIcon icon={faEdit} className="text-brokenblue" fixedWidth />
                    </Link>}
                    {JSON.parse(feature.permissions).includes('d') && <span className="mx-1"><Delete deleteAction={() => this.props.delete(trimester.id)}><FontAwesomeIcon icon={faTrash} className="text-red" fixedWidth /></Delete></span>}
                </div>,
            });
        });
        ...
        const content = (
            <>
                <Row>
                    <List array={data} loading={loading} data={JSON.stringify(trimesters)} get={this.props.get} total={total} bordered add={add} link="/user/trimesters/add" icon={faClock} title={index} className="shadow-sm"
                        fields={[
