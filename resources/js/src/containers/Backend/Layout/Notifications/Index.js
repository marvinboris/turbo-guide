...
export default ({ cms, notifications }) => {
    ...
    const notificationItems = notifications.map(notification => {
        ...
        return <Link key={JSON.stringify(notification)} to={"/restaurant/notifications/" + notification.id} className={"text-dark dropdown-item text-truncate border-top" + (!notification.read_at ? " text-700" : "")}>
            {message}
        </Link>
    });

    return <UncontrolledDropdown inNavbar>
        <DropdownToggle nav className="text-24 p-0 m-0 position-relative">
            <FontAwesomeIcon icon={faBell} className="text-secondary" />
            <Badge color="orange" className="position-absolute rounded-circle text-x-small text-700 d-inline-flex justify-content-center align-items-center" style={{ width: 18, height: 18, top: 0, right: 0, transform: 'translateX(50%)' }}><b className="text-white">{unreadNotifications.length}</b></Badge>
        </DropdownToggle>
        ...
        <DropdownMenu right style={{ width: '20rem' }}>
            {notifications.length === 0 ? <DropdownItem disabled className="bg-dark text-white">
                ...
                <DropdownItem className="text-center pb-0 border-top">
                    <Link className="text-reset small" to={"/restaurant/notifications"}>{cms.header.view_all_notifications}</Link>
                </DropdownItem>
            </>}
        </DropdownMenu>
