...
class Index extends Component {
    ...
    render() {
        ...
        const feature = features.find(f => f.prefix === 'classrooms');
        ...
        const data = classrooms.map(classroom => {
            return updateObject(classroom, {
                action: <div className="text-center">
                    <Link to={`/user/classrooms/${classroom.id}`} className="mr-2">
                        <FontAwesomeIcon icon={faEye} className="text-green" fixedWidth />
                    </Link>
                    {JSON.parse(feature.permissions).includes('u') && <Link to={`/user/classrooms/${classroom.id}/edit`} className="mx-1">
                        <FontAwesomeIcon icon={faEdit} className="text-blue" fixedWidth />
                    </Link>}
                    {JSON.parse(feature.permissions).includes('d') && <span className="mx-1"><Delete deleteAction={() => this.props.delete(classroom.id)}><FontAwesomeIcon icon={faTrash} className="text-red" fixedWidth /></Delete></span>}
                </div>,
            });
        });
        ...
        const content = (
            <>
                <Row>
                    <List array={data} loading={loading} data={JSON.stringify(classrooms)} get={this.props.get} total={total} bordered add={add} link="/user/classrooms/add" icon={faChalkboardTeacher} title={index} className="shadow-sm"
                        fields={[
