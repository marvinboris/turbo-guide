...
class Index extends Component {
    ...
    render() {
        ...
        const feature = features.find(f => f.prefix === 'students');
        ...
        const data = students.map(student => {
            return updateObject(student, {
                action: <div className="text-center">
                    <Link to={`/user/students/${student.id}`} className="mr-2">
                        <FontAwesomeIcon icon={faEye} className="text-green" fixedWidth />
                    </Link>
                    {JSON.parse(feature.permissions).includes('u') && <Link to={`/user/students/${student.id}/edit`} className="mx-1">
                        <FontAwesomeIcon icon={faEdit} className="text-blue" fixedWidth />
                    </Link>}
                    {JSON.parse(feature.permissions).includes('d') && <span className="mx-1"><Delete deleteAction={() => this.props.delete(student.id)}><FontAwesomeIcon icon={faTrash} className="text-red" fixedWidth /></Delete></span>}
                </div>,
            });
        });
        ...
        const content = (
            <>
                <Row>
                    <List array={data} loading={loading} data={JSON.stringify(students)} get={this.props.get} total={total} bordered add={add} link="/user/students/add" icon={faUserGraduate} title={index} className="shadow-sm"
                        fields={[
