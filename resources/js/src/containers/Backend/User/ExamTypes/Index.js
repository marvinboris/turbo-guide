...
class Index extends Component {
    ...
    render() {
        ...
        const data = exam_types.map(exam_type => {
            return updateObject(exam_type, {
                action: <div className="text-center">
                    <Link to={`/user/exam-types/${exam_type.id}`} className="mr-2">
                        <FontAwesomeIcon icon={faEye} className="text-green" fixedWidth />
                    </Link>
                    {JSON.parse(feature.permissions).includes('u') && <Link to={`/user/exam-types/${exam_type.id}/edit`} className="mx-1">
                        <FontAwesomeIcon icon={faEdit} className="text-blue" fixedWidth />
                    </Link>}
                    {JSON.parse(feature.permissions).includes('d') && <span className="mx-1"><Delete deleteAction={() => this.props.delete(exam_type.id)}><FontAwesomeIcon icon={faTrash} className="text-red" fixedWidth /></Delete></span>}
                </div>,
            });
        });
        ...
        const content = (
            <>
                <Row>
                    <List array={data} loading={loading} data={JSON.stringify(exam_types)} get={this.props.get} total={total} bordered add={add} link="/user/exam_types/add" icon={faListOl} title={index} className="shadow-sm"
                        fields={[
